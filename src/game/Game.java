...
public class Game {
    ...
    public Game(int[][] board, int free) {
        this.board = board;
    ...
    public Game move(Direction direction) {
        ...
        return new Game(newBoard, free + growths);
    }
    ...
    public Game placeRandom() {
        int[][] newBoard = deepCopyBoard();
        placeRandom(newBoard);
        return new Game(newBoard, free-1);
    }

    private void placeRandom(int[][] newBoard) {
        int p = random.nextInt(free);
        int val = (random.nextInt(2) + 1) * 2; // 2 or 4
        int freeCount = 0;
